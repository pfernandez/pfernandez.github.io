# The Program That Already Happened
...
Concretely: `(I a)` — apply the literal identity function — settles at `a`. `((I I) a)` — apply the *computed* identity function — used to freeze. The stitcher found function positions by walking left until it reached something settled, and if the settled thing was an answer rather than a definition, it stopped, politely, and left the application as sculpture. The observer, pointed at the sculpture, fell through the answer to the `I` inside and never applied it. In that universe the past was citable — the knot depended on citing answers by pointer — but it was closed. You could point at a settled event. You could not reopen it and use what was inside as the cause of something new.

The fork is one rule: should stitch look through settled answers into their payloads?

Take that branch and you get true normalization. Computed functions apply; `((I I) a)` is `a`; pipelines compose by value instead of by spine; the compiler graduates from specializer toward supercompiler. It is, plainly, the more powerful universe.

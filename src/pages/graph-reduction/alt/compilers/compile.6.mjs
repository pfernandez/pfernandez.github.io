...
const parse = tokens => {
  ...
  if (Array.isArray(result.ast) && result.ast[0] === 'let') {
...
const compile = ast => {
  ...
  const connect = (form, [name, node], scope) => {
    ...
    if (!Array.isArray(form) || reaches(form[0], form) ||
        scope.functions.includes(form)) return form
  ...
  if (ast[0] !== 'let') return expression(ast, emptyScope)

  const [bindings, body] = ast.slice(1)
  const scope = bindings.reduce((scope, source) => {
  ...
  return expression(scope.names.reduce((form, pair) =>
    connect(form, pair, scope), body), scope)
}

...
const compile = ast => {
  ...
  const isFunction = form =>
    definitions.includes(form) && Array.isArray(form[0]) && form[0].every(Array.isArray)
  ...
  const flat = form =>
    isSymbol(form) || isRoot(form) || isFunction(form) || form.length === 0
      ? [form]
  ...
  const connect = (form, name, node) => {
    ...
    if (isSymbol(form) || isRoot(form) || isFunction(form)) return form
  ...
  const copy = (form, pairs) => {
    ...
    return pair
      ? pair[1]
      : isSymbol(form) || isRoot(form) || isFunction(form)
        ? form
        : form.map(item => copy(item, pairs))
  }

  const expression = form => {
    if (isSymbol(form) || isRoot(form) || isFunction(form) || form.length === 0) return form

    const [head, ...args] = flat(form).map(expression)
    const params = isFunction(head) ? head[0] : []

    if (!isFunction(head) || args.length < params.length) return apply([head, ...args])
    ...
    root[0] = focus
    root[1] = expression(copy(body, params.map((param, i) => [param, args[i]])))
    active.pop()

...
const parse = tokens => {
  ...
  const check = ast =>
    Array.isArray(ast) && ast[0] === 'let'
      ? Array.isArray(ast[1])
        ? (ast[1].map(binding), ast)
        : err('Let bindings must be a list')
      : ast

  const close = ({ ast, stack }, token) =>
    stack.length === 0

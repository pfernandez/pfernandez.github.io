// the record as bytes — eight per cell, little-endian; an atom is its own address twice
...
export const image = graph => {
  ...
  const place = node => {
    ...
    if (!Array.isArray(node) || node.length !== 2) throw new Error('Image cells must be pairs')

...
When the graph becomes a file, the metaphysics becomes measurable. The compiler emits a complete WebAssembly module by hand — no toolchain, just bytes. For the library's demo program: 221 bytes total. Eighty of them are the record itself, ten cells. Thirty-nine are the code — the observe loop and the select load. Six are the focus pointer, and those six are identical in every module ever emitted, because the focus is always written first, at address zero. Seventeen are the legend of names, which the machine cannot read.

...
const connect = ast => {
  ...
  const tie = (form, scope) => {
    ...
    // Tie a stable root to the connected body.
    const root = []
...
export const graph = source =>
  connect(curry(parse(tokenize(source))))
...
let N = 1
const trace = (form, clone = (expr, path = '$', seen = new Map()) =>
...
export const observe = (focus, visit = trace) => {
  const step = pair => (
    visit(pair),
    (pair[0] === pair) ? pair[1] : step(pair[0]))
  return step(focus)
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const root = graph(readFileSync('./core.lisp', 'utf-8'))
  trace(observe(root))
}
